...
public class DefaultNoise extends Configurable implements NoiseProducer {
  /**
   * Draws a noise on the image. The noise curve depends on the factor values. Noise won't be
   * visible if all factors have the value &gt; 1.0f
   *
  ...
  public void makeNoise(
    ...
    for (i = 0; i < pts.length - 1; i++) {
      if (i < 3) graph.setStroke(new BasicStroke(0.9f * (4 - i)));
      graph.drawLine(
          (int) pts[i].getX(),
          (int) pts[i].getY(),
          (int) pts[i + 1].getX(),
